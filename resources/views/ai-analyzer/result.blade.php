    ...
    <x-slot name="header">
        <div class="flex flex-col">
            <h1 class="text-2xl font-black text-slate-900 leading-tight tracking-tight">
                Analysis <span class="bg-clip-text text-transparent bg-gradient-to-r from-[#d983e4] via-purple-600 to-[#4e71c5]">Results</span>
            </h1>
    ...
    <div class="bg-[#fcfcfd] min-h-screen pb-24 font-sans">
        <div class="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                ...
                <div class="md:col-span-3 bg-white rounded-[2.5rem] p-8 border border-slate-200/60 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
                    <div class="relative w-28 h-28 mb-4">
                        <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                            ...
                            <circle class="text-indigo-600 transition-all duration-1000 ease-out" stroke-width="8" stroke-dasharray="264" stroke-dashoffset="{{ 264 * (1 - 85/100) }}" stroke-linecap="round" stroke="currentColor" fill="none" r="42" cx="50" cy="50" />
                        </svg>
                ...
                {{-- Key Insights Card --}}
                <div class="md:col-span-9 bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl shadow-indigo-100 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
                    <div class="relative z-10 flex-1">
                        <div class="flex items-center gap-3 mb-3">
                            <i class="ph-bold ph-lightning text-indigo-400 text-lg"></i>
                            <h3 class="text-[10px] font-black uppercase tracking-[2px] text-indigo-300">Executive Summary</h3>
                        </div>
                        <p class="text-base font-medium text-indigo-50 leading-relaxed italic">
                    ...
                    <div class="relative z-10 flex flex-col gap-3 min-w-[200px]">
                        <div class="px-5 py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-3">
                            ...
                            <span class="text-[9px] font-black uppercase tracking-widest text-indigo-100">Quantified Impact</span>
                        </div>
                        <div class="px-5 py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-3">
                            <i class="ph-bold ph-check-circle text-emerald-400"></i>
                            <span class="text-[9px] font-black uppercase tracking-widest text-indigo-100">Keyword Optimized</span>
                        </div>
                    </div>
                    {{-- Decorative blur --}}
                    ...
                    @if(isset($result[$key]) && (!empty($result[$key]['teks_revisi']) || !empty($result[$key]['alasan_perubahan'])))
                        <div class="relative pl-12 sm:pl-16 group">
                            ...
                            <div class="absolute left-0 top-0 w-12 sm:w-16 h-12 sm:h-16 flex items-center justify-center">
                                <div class="w-10 sm:w-12 h-10 sm:h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-indigo-600 group-hover:border-indigo-600/30 transition-all shadow-sm z-10 bg-[#fcfcfd]">
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                    ...
                                    <div class="lg:col-span-5 bg-white rounded-3xl p-6 border border-slate-200/60 shadow-sm">
                                        <div class="flex items-center gap-3 mb-4">
                                            <i class="ph-bold ph-brain text-indigo-600"></i>
                                    ...
                                    <div class="lg:col-span-7 bg-indigo-50/50 rounded-3xl p-6 border border-indigo-100 relative group/card">
                                        <div class="flex items-center justify-between mb-4">
                                            <div class="flex items-center gap-3">
                                                <i class="ph-bold ph-sparkle text-indigo-600"></i>
                                                <h4 class="text-[9px] font-black text-indigo-600 uppercase tracking-widest">Optimized Output</h4>
                                            </div>
                                            <button onclick="copyToClipboard(this)" class="text-indigo-400 hover:text-indigo-600 transition-colors">
            ...
            <div class="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-lg px-4 z-50">
                <div class="bg-slate-900/90 backdrop-blur-xl rounded-3xl p-3 flex items-center gap-3 shadow-2xl border border-white/10">
                    <a href="{{ route('cv.builder') }}" class="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[2px] hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 active:scale-95">
            ...
            formattedContents.forEach((content) => {
                ...
                let formatted = lines.map(line => {
                    ...
                    if (line.startsWith('●') || line.startsWith('•') || line.startsWith('-')) {
                        return `<div class="flex items-start gap-3 mb-2"><span class="text-indigo-600 shrink-0 mt-1.5 font-black">•</span><span>${line.substring(1).trim()}</span></div>`;
                    }
                    
                    const numMatch = line.match(/^\d+\.\s+(.+)/);
                    if (numMatch) {
                        return `<div class="flex items-start gap-4 mb-2"><span class="font-black text-indigo-600 shrink-0 w-5 h-5 bg-indigo-50 rounded-lg flex items-center justify-center text-[10px]">${line.split('.')[0]}</span><span>${numMatch[1]}</span></div>`;
                    }

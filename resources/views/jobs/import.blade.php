<x-app-layout>
    ...
    <style>
        .drag-active { border-color: rgb(165, 112, 240) !important; background-color: rgb(245, 243, 255) !important; }
        .drag-active .upload-icon { background: rgb(165, 112, 240); color: white; }
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div class="lg:col-span-8 space-y-5">
                        ...
                        <div class="px-7 pt-6 pb-4 border-b border-slate-100 flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 bg-primary-50 rounded-xl flex items-center justify-center">
                                    <i class="ph-bold ph-file-arrow-up text-primary-600 text-base"></i>
                                </div>
                                <div>
                            ...
                            <form action="{{ route('csv.import.process') }}" method="POST" enctype="multipart/form-data" id="importForm">
                                ...
                                <div id="upload-area" class="relative border-2 border-dashed border-slate-200 rounded-2xl p-10 text-center cursor-pointer transition-all duration-200 overflow-hidden"
                                    ...
                                    <div class="relative z-10 flex flex-col items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-black text-slate-800">Drop CSV here or <span class="text-primary-600">browse files</span></p>
                                ...
                                <div class="flex items-center justify-between mt-6 pt-5 border-t border-slate-100">
                                    ...
                                    <button type="submit" id="submit-btn"
                                            class="flex items-center gap-2 px-7 py-3 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-primary-600 transition-all shadow-xl shadow-slate-200 active:scale-95">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-sm">
                            <div class="flex items-center gap-2 mb-3">
                                <i class="ph-fill ph-plus-circle text-primary-500 text-base"></i>
                                <h4 class="text-[10px] font-black text-primary-600 uppercase tracking-widest">Optional Columns</h4>
                            </div>
                            <div class="flex flex-wrap gap-1.5">
                                @foreach(['Platform Link', 'Interview Date', 'Interview Type', 'Notes'] as $f)
                                <span class="px-2.5 py-1 bg-primary-50 border border-primary-100 rounded-lg text-[10px] font-black text-primary-600">{{ $f }}</span>
                                @endforeach
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="w-full text-[10px] font-mono">
                                <thead>
                                    <tr class="border-b border-slate-800">
                                        ...
                                        <th class="px-4 py-2.5 text-left text-primary-400 font-bold whitespace-nowrap">{{ $col }}</th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody>
                ...
                <div class="lg:col-span-4 space-y-5">
                    ...
                    {{-- Quick guide --}}
                    <div class="bg-primary-600 rounded-3xl p-6 text-white relative overflow-hidden shadow-xl shadow-primary-100">
                        <div class="absolute -right-8 -bottom-8 w-32 h-32 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>
                        <div class="absolute -left-4 -top-4 w-20 h-20 bg-primary-400/20 rounded-full blur-xl pointer-events-none"></div>
                        <div class="relative z-10">
                            <p class="text-[10px] font-black text-primary-200 uppercase tracking-widest mb-4">Quick Guide</p>
                            <div class="space-y-4">
                                @foreach([
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="w-8 h-8 bg-white/10 border border-white/10 rounded-xl flex items-center justify-center shrink-0">
                                        <i class="ph-bold {{ $icon }} text-primary-200 text-sm"></i>
                                    </div>
                                    <div>
                                        <p class="text-xs font-black text-white">{{ $title }}</p>
                                        <p class="text-[10px] text-primary-200/70 mt-0.5 leading-snug">{{ $desc }}</p>
                                    </div>
                                </div>
                                @endforeach
                            </div>

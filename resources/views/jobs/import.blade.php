<x-app-layout>
    ...
    <div class="bg-[#f8fafc] min-h-screen pb-20" x-data="{ 
        currentStep: 1, 
        fileName: '', 
        isScanning: false, 
        scanProgress: 0,
            ...
            <div class="mb-12 max-w-5xl mx-auto">
                <div class="relative flex justify-between items-center">
                    ...
                    <div class="relative z-10 flex flex-col items-center gap-4">
                        <div class="w-14 h-14 rounded-2xl border-[6px] border-[#f8fafc] flex items-center justify-center transition-all duration-500"
                            ...
                            <i class="ph-bold {{ $step['icon'] }} text-xl transition-transform" :class="currentStep == {{ $i+1 }} ? 'scale-125' : ''"></i>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                ...
                <div class="lg:col-span-8">
                    ...
                    <div x-show="currentStep === 1" class="space-y-6" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0">
                        <div class="bg-white rounded-[3rem] border border-slate-200/60 shadow-[0_20px_50px_rgba(0,0,0,0.03)] overflow-hidden relative">
                            <div class="absolute top-0 right-0 w-64 h-64 bg-primary-50 rounded-full blur-[80px] -mr-32 -mt-32 opacity-60"></div>
                            
                            <div class="px-10 py-8 border-b border-slate-50 flex items-center justify-between relative z-10">
                                <div class="flex items-center gap-5">
                                    <div class="w-14 h-14 rounded-[1.5rem] bg-slate-900 flex items-center justify-center text-white shadow-xl shadow-slate-200">
                                        <i class="ph-duotone ph-cloud-arrow-up text-3xl"></i>
                                    </div>
                                    <div>
                                        <h3 class="text-xl font-black text-slate-900 tracking-tight">Ingestion Hub</h3>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Select your CSV or TXT data source</p>
                                    </div>
                                </div>
                                <a href="{{ route('csv.template') }}" class="group flex items-center gap-2.5 px-6 py-3 bg-emerald-50 text-emerald-700 rounded-2xl font-black text-[11px] uppercase tracking-widest border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                            ...
                            <div class="p-10 relative z-10">
                                <template x-if="!isScanning">
                                    <div id="drop-zone" 
                                         class="relative border-4 border-dashed border-slate-100 rounded-[2.5rem] p-20 text-center cursor-pointer transition-all duration-500 hover:border-primary-200 hover:bg-primary-50/10 group"
                                        ...
                                        <div class="upload-icon flex flex-col items-center gap-6 transition-all duration-500">
                                            <div class="w-24 h-24 rounded-[2rem] bg-white flex items-center justify-center text-slate-300 border border-slate-100 shadow-xl group-hover:shadow-primary-100 group-hover:text-primary-600 group-hover:border-primary-200 transition-all duration-500">
                                                <i class="ph-bold ph-file-csv text-5xl"></i>
                                            </div>
                                            <div class="space-y-2">
                                                <p class="text-2xl font-black text-slate-900 tracking-tight">Drop your batch here</p>
                                                <p class="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">or browse your professional archive</p>
                                            </div>
                                        </div>
                                    </div>
                                </template>
                                ...
                                <template x-if="isScanning">
                                    <div class="py-20 flex flex-col items-center justify-center space-y-8 animate-pulse">
                                        <div class="relative w-24 h-24">
                                            ...
                                            <div class="absolute inset-0 flex items-center justify-center">
                                                <i class="ph-fill ph-magnifying-glass text-3xl text-primary-600"></i>
                                            </div>
                                        </div>
                                        <div class="text-center">
                                            <p class="text-xl font-black text-slate-900 tracking-tight">Scanning Structure...</p>
                    ...
                    <div x-show="currentStep === 2" x-cloak class="space-y-8" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-x-12" x-transition:enter-end="opacity-100 translate-x-0">
                        {{-- Mapping Controls --}}
                        ...
                        <div class="bg-white rounded-[3rem] border border-slate-200/60 shadow-2xl shadow-slate-200/50 overflow-hidden">
                            ...
                            <div class="p-10 bg-white flex items-center justify-between">
                                <button @click="currentStep = 1" class="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-all">
                                    <i class="ph-bold ph-arrow-left"></i> Re-Upload
                                </button>
                                <form action="{{ route('csv.import.process') }}" method="POST" enctype="multipart/form-data" id="finalForm">
                                    @csrf
                                    {{-- Hidden inputs for real processing --}}
                                    <button type="submit" class="relative overflow-hidden px-12 py-5 bg-primary-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-primary-700 transition-all shadow-2xl shadow-primary-200 active:scale-95 group flex items-center gap-3">

<aside 
    :class="{ 
        'w-20': !$store.sidebar.open && !mobileSidebarOpen, 
        'w-72': $store.sidebar.open || mobileSidebarOpen,
...
>
    ...
    <button @click="$store.sidebar.toggle()" 
        ...
        <i class="ph ph-caret-left text-[11px] transition-transform duration-300" :class="($store.sidebar.open || mobileSidebarOpen) ? '' : 'rotate-180'"></i>
    </button>
    ...
    <div class="flex items-center h-[72px] px-6 border-b border-slate-100 flex-shrink-0 relative overflow-hidden bg-white/50 backdrop-blur-sm">
        <div class="flex items-center space-x-3 relative z-10 w-full" :class="($store.sidebar.open || mobileSidebarOpen) ? '' : 'justify-center space-x-0'">
            ...
            <div x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="flex flex-col truncate">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto sidebar-scroll">
        <!-- Section Label -->
        <div x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="px-2 mb-3 mt-2">
        ...
        <a href="{{ route($link['route']) }}" wire:navigate
           ...
           :class="($store.sidebar.open || mobileSidebarOpen) ? '' : 'justify-center'">
            ...
            @endif
            <div class="flex items-center justify-center flex-shrink-0 relative z-10" :class="($store.sidebar.open || mobileSidebarOpen) ? 'w-8' : ''">
            ...
            <span x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="ml-3 truncate">{{ $link['label'] }}</span>
            <!-- Tooltip -->
        ...
        <div x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="px-2 mb-3">
        ...
        <a href="{{ route('admin.index') }}" wire:navigate
           ...
           :class="($store.sidebar.open || mobileSidebarOpen) ? '' : 'justify-center'">
            ...
            <span x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="ml-3 truncate">Admin Panel</span>
            <div x-show="!$store.sidebar.open" class="absolute left-14 px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-md shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300 z-50 whitespace-nowrap">
                Admin Panel
                <div class="absolute right-full top-1/2 -translate-y-1/2 border-4 border-transparent border-r-slate-800"></div>
            </div>
    ...
    <div class="p-4 border-t border-slate-100 bg-slate-50/50">
        <div class="flex items-center bg-white rounded-xl p-2 cursor-pointer hover:shadow-sm border border-slate-200/60 transition-all duration-200"
            ...
            <div x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms class="ml-3 min-w-0 flex-1">
        ...
        <form x-show="$store.sidebar.open || mobileSidebarOpen" x-transition.opacity.duration.300ms method="POST" action="{{ route('logout') }}" class="mt-2" id="logout-form-sidebar">
            @csrf

        ...
        <div class="flex items-center space-x-3.5">
            ...
            <div class="relative shrink-0" x-data="{ open: false }">
                ...
                <div x-show="open" 
                     ...
                     class="absolute right-0 top-full mt-2 w-64 bg-white/98 backdrop-blur-md rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.1),0_1px_3px_rgba(0,0,0,0.03)] border border-zinc-200/75 z-50 text-left"
                     style="display: none;">

                    <!-- Account Info Header & Gamification Level -->
                    <div class="px-4 pt-4 pb-3 bg-zinc-50/50 border-b border-zinc-100/80 rounded-t-xl">
                        <div class="sm:hidden mb-2.5 pb-2.5 border-b border-zinc-200/60">
                            <p class="text-[8px] font-bold text-zinc-400 uppercase tracking-widest leading-none">Signed in as</p>
                            <p class="text-xs font-bold text-zinc-800 truncate leading-none mt-1.5">{{ Auth::user()->name }}</p>
                            <p class="text-[10px] font-medium text-zinc-400 truncate leading-none mt-1">{{ Auth::user()->email }}</p>
                        </div>
                        <p class="text-[9.5px] font-bold text-zinc-400 uppercase tracking-widest leading-none">Account Rewards</p>
                    ...
                    <div class="p-2 space-y-1">
                        <a href="{{ route('profile.edit') }}" wire:navigate class="group flex items-center justify-between px-3 py-2.2 text-xs font-medium text-zinc-700 hover:text-zinc-950 hover:bg-zinc-50 border border-transparent hover:border-zinc-150/30 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2.5">
                                <i class="ph ph-user text-base text-zinc-400 group-hover:text-zinc-600 transition-colors"></i>
                                <span>My Profile</span>
                            </div>
                            <i class="ph ph-caret-right text-[10px] text-zinc-350 opacity-0 group-hover:opacity-100 transition-all duration-150 transform group-hover:translate-x-0.5"></i>
                        </a>

                        @if(!Auth::user()->isPremium())
                        <a href="{{ route('payment.premium') }}" wire:navigate class="group flex items-center justify-between px-3 py-2.2 text-xs font-semibold text-primary-700 bg-primary-50/45 hover:bg-primary-50 border border-primary-200/40 hover:border-primary-200/80 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2.5">
                                <i class="ph-fill ph-crown text-base text-primary-500"></i>
                                <span>Upgrade Premium</span>
                            </div>
                            <span class="text-[8.5px] font-bold uppercase bg-primary-100 text-primary-850 px-1.5 py-0.5 rounded shadow-2xs select-none">Pro</span>
                        </a>
                        @else
                        <div class="flex items-center justify-between px-3 py-2.2 text-xs font-semibold text-amber-800 bg-amber-50/50 border border-amber-200/60 rounded-lg select-none">
                            <div class="flex items-center gap-2.5">
                                <i class="ph-fill ph-crown text-base text-amber-500 animate-pulse"></i>
                                <span>Pro Member</span>
                            </div>
                            <span class="text-[8.5px] font-black uppercase bg-amber-100 text-amber-900 px-1.5 py-0.5 rounded shadow-2xs">Pro</span>
                        </div>
                        @endif

                        <a href="{{ route('automation.index') }}" wire:navigate class="group flex items-center justify-between px-3 py-2.2 text-xs font-medium text-zinc-700 hover:text-zinc-950 hover:bg-zinc-50 border border-transparent hover:border-zinc-150/30 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2.5">
                        ...
                        <a href="{{ route('support.index') }}" wire:navigate class="group flex items-center justify-between px-3 py-2.2 text-xs font-medium text-zinc-700 hover:text-zinc-950 hover:bg-zinc-50 border border-transparent hover:border-zinc-150/30 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2.5">
                    ...
                    <div class="h-[1px] bg-zinc-100 mx-2 my-1.5"></div>

                    <!-- Log Out Action -->
                    <div class="p-2 pt-0">
                        <form method="POST" action="{{ route('logout') }}" id="logout-form">
                            @csrf
                            <button type="button" onclick="confirmLogout()" class="w-full group flex items-center justify-between px-3 py-2.2 text-xs font-semibold text-rose-600 hover:bg-rose-50/50 hover:text-rose-700 rounded-lg border border-transparent transition-all duration-150">
                                <div class="flex items-center gap-2.5">

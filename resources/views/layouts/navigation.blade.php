        ...
        <div class="flex items-center space-x-3.5">
            ...
            <div class="relative shrink-0" x-data="{ open: false }">
                ...
                <div x-show="open" 
                     ...
                     class="absolute right-0 top-full mt-2 w-56 bg-white/95 backdrop-blur-md rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.02)] border border-zinc-200/50 z-50 text-left"
                     style="display: none;">

                    <!-- Account Info Header & Gamification Level -->
                    <div class="px-3.5 pt-3.5 pb-2.5 bg-zinc-50/60 border-b border-zinc-100 rounded-t-xl">
                        ...
                        <p class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider leading-none">Account Rewards</p>
                    ...
                    <div class="p-1.5 space-y-0.5">
                        <a href="{{ route('profile.edit') }}" wire:navigate class="group flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-zinc-600 hover:text-zinc-950 hover:bg-zinc-50 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-user-circle text-base text-zinc-400 group-hover:text-zinc-650 transition-colors"></i>
                                <span>My Profile</span>
                            </div>
                            <i class="ph ph-caret-right text-[10px] text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-150"></i>
                        </a>

                        @if(!Auth::user()->isPremium())
                        <a href="{{ route('payment.premium') }}" wire:navigate class="group flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-primary-600 hover:bg-primary-50/50 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-crown text-base text-primary-500"></i>
                                <span>Upgrade Premium</span>
                            </div>
                            <span class="text-[8px] font-black uppercase bg-primary-100 text-primary-800 px-1 py-0.2 rounded-sm select-none">Pro</span>
                        </a>
                        @else
                        <div class="flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-amber-700 bg-amber-50/40 border border-amber-100/50 rounded-lg">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-crown text-base text-amber-500"></i>
                                <span>Pro Member</span>
                            </div>
                            <span class="text-[8px] font-black uppercase bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded-sm select-none">Pro</span>
                        </div>
                        @endif

                        <a href="{{ route('automation.index') }}" wire:navigate class="group flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-zinc-600 hover:text-zinc-950 hover:bg-zinc-50 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-sliders text-base text-zinc-400 group-hover:text-zinc-650 transition-colors"></i>
                                <span>Automation</span>
                            </div>
                            <i class="ph ph-caret-right text-[10px] text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-150"></i>
                        </a>

                        <a href="{{ route('support.index') }}" wire:navigate class="group flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-zinc-600 hover:text-zinc-950 hover:bg-zinc-50 rounded-lg transition-all duration-150">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-headset text-base text-zinc-400 group-hover:text-zinc-650 transition-colors"></i>
                                <span>Support</span>
                            </div>
                            <i class="ph ph-caret-right text-[10px] text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity duration-150"></i>
                        </a>
                    ...
                    <div class="h-[1px] bg-zinc-100/80 mx-1.5 my-1"></div>

                    <!-- Log Out Action -->
                    <div class="p-1.5 pt-0">
                        <form method="POST" action="{{ route('logout') }}" id="logout-form">
                            @csrf
                            <button type="button" onclick="confirmLogout()" class="w-full group flex items-center justify-between px-2.5 py-1.8 text-[11px] font-semibold text-rose-600 hover:bg-rose-50/50 hover:text-rose-700 rounded-lg transition-all duration-150">
                                <div class="flex items-center gap-2">

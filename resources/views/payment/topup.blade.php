    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <h1 class="text-base font-semibold text-zinc-900 tracking-tight">{{ $packageName }}</h1>
                <span class="px-2 py-0.5 bg-primary-50 text-zinc-800 border border-primary-200/60 rounded text-[10px] font-bold tracking-wide uppercase">Add-On Pack</span>
            </div>
            <div class="flex items-center gap-2">
                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200/60 text-[10px] font-bold uppercase tracking-wider">
                    ...
                    Instant Credit Credit Injection
                </span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-[1100px] mx-auto px-4 py-6" x-data="{ selectedMethod: '' }">
        ...
        <div class="flex items-center justify-center gap-3 mb-6 bg-white border border-zinc-200/80 rounded-lg p-2.5 max-w-xl mx-auto shadow-3xs">
            ...
            <div class="flex items-center gap-2 opacity-50">
                ...
                <span class="text-xs font-medium text-zinc-500 tracking-tight">Kredit Bertambah</span>
            </div>
        </div>
            ...
            <div class="lg:col-span-5">
                <div class="sticky top-20 bg-white border border-zinc-200/80 rounded-lg p-4 space-y-4 shadow-3xs">
                    ...
                    <div class="flex items-center justify-between pb-3 border-b border-zinc-100">
                        ...
                        <span class="text-[10px] font-bold text-primary-700 bg-primary-50 border border-primary-200/60 px-2 py-0.5 rounded">Top Up Add-On</span>
                    </div>
                    ...
                    <div class="p-3.5 bg-zinc-50/60 rounded-lg border border-zinc-200/60 space-y-3">
                        <div class="flex justify-between items-start pb-2.5 border-b border-zinc-200/60">
                            <div>
                                <p class="text-xs font-bold text-zinc-900 tracking-tight">{{ $packageName }}</p>
                                <p class="text-[10px] text-zinc-500 font-medium mt-0.5">{{ $packageSubtitle }}</p>
                            </div>
                            <span class="text-xs font-bold text-zinc-900 tracking-tight shrink-0">Rp {{ number_format($addonPrice, 0, ',', '.') }}</span>
                        </div>

                        <div class="space-y-1.5 pt-0.5">
                            <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Kredit Langsung Aktif:</p>
                            <div class="grid grid-cols-1 gap-1 text-[11px] font-medium text-zinc-700">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>Top-Up Saldo AI Credits Tambahan</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <i class="ph-bold ph-check text-emerald-600 text-xs shrink-0"></i>
                                    <span>Tanpa Kedaluwarsa (Kredit Permanen)</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <i class="ph-bold ph-check text-emerald-600 text-xs shrink-0"></i>
                                    <span>Berlaku untuk AI Studio Engines</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 px-1 text-xs">
                        <div class="flex justify-between font-medium text-zinc-500">
                            ...
                            <span class="text-zinc-800 font-semibold">Rp {{ number_format($addonPrice, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="pt-2 border-t border-zinc-200/80 flex justify-between items-center text-sm">
                            ...
                            <span class="font-black text-primary-600 tracking-tight text-base">Rp {{ number_format($addonPrice, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    ...
                    <button type="submit" form="paymentForm" id="btn-submit-payment"
                        ...
                        <span id="text-submit-payment">Konfirmasi & Tambah Kredit</span>
                    </button>

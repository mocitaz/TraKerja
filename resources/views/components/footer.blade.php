<footer class="bg-white border-t border-gray-200 py-4 sm:py-6 mt-auto">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
            <!-- Brand -->
            <div class="flex items-center gap-2 sm:gap-3">
                <img src="{{ asset('images/icon.png') }}" 
                     alt="TraKerja Logo" 
                     class="h-6 w-6 sm:h-8 sm:w-8"
                     onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                <div class="h-6 w-6 sm:h-8 sm:w-8 bg-gradient-to-br from-primary-500 to-primary-700 rounded-lg flex items-center justify-center" style="display: none;">
                    <span class="text-white font-bold text-xs sm:text-sm">T</span>
                </div>
                <span class="text-sm sm:text-base font-semibold bg-gradient-to-r from-[#d983e4] to-[#4e71c5] bg-clip-text text-transparent">TraKerja</span>
            </div>

            <!-- Copyright -->
            <div class="text-xs sm:text-sm text-gray-600 text-center sm:text-right">
                © 2025 TraKerja. <a href="https://www.instagram.com/teknalogi.id/" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 font-medium transition-colors duration-200">PT Teknologi Transformasi Digital</a>. All rights reserved.
            </div>
        </div>
    </div>
</footer>





        ...
        <div class="w-full max-w-[340px] mx-auto my-auto flex flex-col items-center space-y-6">
            ...
            <div class="w-full">
                @if (session('status') === 'registration-successful' || session('status') === 'verification-link-sent')
                    <div class="mb-4 p-3 bg-amber-50/50 border border-amber-200/80 rounded-md text-amber-900 text-xs font-semibold flex items-start gap-2 text-left">
                ...
                @if (session('status') === 'email-not-verified')
                    <div class="mb-4 p-3 bg-rose-50/50 border border-rose-200/80 rounded-md text-rose-900 text-xs font-semibold flex items-start gap-2 text-left">
            ...
            <!-- Form -->
            <form method="POST" action="{{ route('login') }}" class="w-full space-y-4 text-left">
                @csrf
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="password" class="block text-[11px] font-medium text-slate-550 mb-0">Password</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[11px] text-slate-400 hover:text-slate-655 transition-colors">
                                Forgot?
                            </a>
                        @endif
                    </div>
